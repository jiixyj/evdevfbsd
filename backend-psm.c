...
static int32_t
synaptics_inverse_y(int32_t y)
{
	return Y_MAX_DEFAULT + Y_MIN_DEFAULT - y;
}

static int
synaptics_setup_abs_axes(
    struct event_device *ed, struct psm_backend *b, int x_axis, int y_axis)
{
	set_bit(ed->abs_bits, x_axis);
	set_bit(ed->abs_bits, y_axis);
	if (b->synaptics_info.minimumXCoord &&
	...
	if (b->synaptics_info.maximumXCoord &&
...
static int
write_full_packet(int fd, unsigned char *pkt, size_t siz)
{
...
static void
synaptic_parse_ew_packet(struct event_device *ed, unsigned char *packet)
{
	struct psm_backend *b = ed->priv_ptr;
...
{
	struct psm_backend *b = ed->priv_ptr;
	struct event_times const *tv = &ed->packet_time;
	int buttons = 0;
	...
	if (no_fingers > 0) {
		...
		if (y > 1)
			put_event(
			    ed, tv, EV_ABS, ABS_Y, synaptics_inverse_y(y));
	}
